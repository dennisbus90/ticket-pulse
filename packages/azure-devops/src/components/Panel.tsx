...
import { formatDuration, formatDateShort } from "@ticket-pulse/shared";
...
function EstimationTab({
  ...
  if (!estimation) {
    return (
      ...
      >
        Click analyze to compare estimation against similar work items.
      </div>
    );
  }
      ...
      {estimation.suggested !== null && estimation.teamSet !== null && (
        ...
        >
          {estimation.teamSet === estimation.suggested
            ...
            : `Team estimated ${estimation.teamSet}, but similar items suggest ${estimation.suggested}.`}
        </div>
      )}
      ...
      {estimation.similarTickets.length > 0 && (
        <div>
          ...
          >
            ...
            Similar items ({estimation.similarTickets.length})
          </button>
          {showSimilar && (
...
function TimelineTab({
  ...
  if (!timeline || timeline.transitions.length === 0) {
    return (
      ...
      >
        No status transitions recorded for this work item.
      </div>
    );
  }
...
export const Panel: React.FC<PanelProps> = ({
  ...
  const handleUseThis = async (index: number, finding: Finding) => {
    ...
    const match = analysisFields.find(
      (f) => f.fieldName.toLowerCase() === finding.field.toLowerCase(),
    );
    if (!match) return;
    setFieldUpdateStatus((prev) => ({ ...prev, [index]: "saving" }));
    try {
      await onUpdateField(match.fieldId, finding.suggestion);
        ...
        {activeTab === "quality" && (
          <>
            ...
            {!analysis && !error && (
              ...
              >
                Click analyze to review this work item with AI.
              </div>
            )}
          </>
        )}
