...
import { getTextFields, getEstimationFields } from "../api/fields";
...
interface SettingsProps {
  ...
  onSaveFields: (fields: FieldMapping[]) => Promise<void>;
  onSaveEstimationField: (field: EstimationFieldConfig | null) => Promise<void>;
  proxyUrl: string;
  onSaveProxyUrl: (url: string) => Promise<void>;
  onClose: () => void;
}
...
export const Settings: React.FC<SettingsProps> = ({
  ...
  const [availableEstimationFields, setAvailableEstimationFields] = useState<FieldOption[]>([]);
  ...
  useEffect(() => {
    if (import.meta.env.DEV) {
      setAvailableFields([
        ...
        { id: "Custom.AcceptanceCriteria", name: "Acceptance Criteria" },
        { id: "Custom.UserStory", name: "User Story" },
        { id: "Custom.DefinitionOfDone", name: "Definition of Done" },
      ]);
      setAvailableEstimationFields([
        { id: "Microsoft.VSTS.Scheduling.StoryPoints", name: "Story Points" },
        { id: "Microsoft.VSTS.Scheduling.Effort", name: "Effort" },
      ]);
    ...
    Promise.all([
      ...
      getEstimationFields()
        .then((fields) => setAvailableEstimationFields(fields))
        .catch(() => setAvailableEstimationFields([])),
  ...
  const handleSaveProxy = () => {
    onSaveProxyUrl(proxyInput);
    setMessage({ text: "Proxy URL saved", type: "ok" });
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "10px 14px" }}>
        ...
        >
          Override the default analyze service with your own Azure Function. Leave empty to use the built-in service.
        </div>
        ...
        >
          ...
          >
            Add the fields you want the AI to review.
          </div>
        ...
        >
          ...
          <select
            value={estimationField?.fieldId ?? ""}
            onChange={(e) => {
              const fieldId = e.target.value;
              if (!fieldId) {
                onSaveEstimationField(null);
              } else {
                const selected = availableEstimationFields.find((f) => f.id === fieldId);
                onSaveEstimationField({
                  fieldId,
                  fieldName: selected?.name ?? fieldId,
                });
              }
            }}
            disabled={fieldsLoading}
          ...
          >
            ...
            {availableEstimationFields.map((f) => (
              <option key={f.id} value={f.id}>
                {f.name} ({f.id})
              </option>
            ))}
        ...
        >
          Keys are stored in Azure DevOps Extension Data Service (per-user scope).
        </div>
      </div>
    </div>
  );
